...
const postLink =
    "https://danbooru.donmai.us/posts.json?limit=20&tags=rating:general+";
const tagsLink =
    "https://danbooru.donmai.us/tags.json?search[order]=count&search[name_matches]=";
var tag = "";
var page = 1;
var id = 0;
var ext = "";
...
async function loadImages(tag, page) {
    ...
    if (JSON.length == 0) {
        $("#images").append(`<h1>Well, that sucks.</h1>
    ...
    JSON.forEach((element) => {
        if (
            element.file_ext != "jpg" &&
            element.file_ext != "png" &&
            element.file_ext != "gif"
        ) {
            return;
        }
